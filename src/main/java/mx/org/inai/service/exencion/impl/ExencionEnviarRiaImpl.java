...
/**
 * The Class ExencionEnviarRia.
 *
 * @author A. Juarez
 */
@Strategy(tipoExencion = TipoExencion.ENVIAR_RIA)
public class ExencionEnviarRiaImpl extends ExencionAbstract<String> {
	...
	public ExencionResponseDTO<String> registrar(ExencionRequestDTO peticion, String folio) {
		...
		try {
			String etapa = folioService.obtenerEtapa(KEY_ETAPAH5);
			peticion.setEtapa(etapa);
			peticion.setNumDias(5);
			peticion.setTipoFlujo(TipoFlujo.EXENCION);
			peticion.setTipoEnvio(TipoEnvio.ENVIAR_RIA);
