...
/**
 * The Class ExencionRegistrarRia.
 *
 * @author A. Juarez
 */
@Strategy(tipoExencion = TipoExencion.REGISTRAR_RIA)
public class ExencionRegistrarRiaImpl extends ExencionAbstract<String> {
	...
	public ExencionResponseDTO<String> registrar(ExencionRequestDTO peticion, String folio) {
		...
		try {
			String etapa = folioService.obtenerEtapa(KEY_ETAPAH4);
			peticion.setEtapa(etapa);
			peticion.setNumDias(5);
			peticion.setTipoFlujo(TipoFlujo.EXENCION);
			peticion.setTipoEnvio(TipoEnvio.RIA);
